...
description: Setting up GovSense using Docker
---
...
This guide explains how to run GovSense using Docker, with options ranging from quick single-command deployment to full production setups.

## Quick Start with Docker 🐳

Get GovSense running in seconds with a single command:

<Callout type="info">
The all-in-one Docker image bundles PostgreSQL (with pgvector), Redis, and all GovSense services. Perfect for quick evaluation and development.
</Callout>

<Callout type="warn">
Make sure to include the `-v govsense-data:/data` in your Docker command. This ensures your database and files are properly persisted.
</Callout>
...
curl -o docker-compose.yml https://raw.githubusercontent.com/MODSetter/GovSense/main/docker-compose.quickstart.yml
...
```bash
# View logs
docker logs -f govsense

# Stop GovSense
docker stop govsense

# Start GovSense
docker start govsense

# Remove container (data preserved in volume)
docker rm govsense

# Remove container AND data
docker rm govsense && docker volume rm govsense-data
```
...
1. **Configure Environment Variables**
    ...
    **Linux/macOS:**
    ...
    cp govsense_backend/.env.example govsense_backend/.env
    cp govsense_web/.env.example govsense_web/.env
    ...
    **Windows (Command Prompt):**
    ...
    ```cmd
    copy govsense_backend\.env.example govsense_backend\.env
    copy govsense_web\.env.example govsense_web\.env
    ...
    **Windows (PowerShell):**
    ...
    ```powershell
    Copy-Item -Path govsense_backend\.env.example -Destination govsense_backend\.env
    Copy-Item -Path govsense_web\.env.example -Destination govsense_web\.env
...
| POSTGRES_DB                | PostgreSQL database name                                                    | govsense           |
| PGADMIN_DEFAULT_EMAIL      | Email for pgAdmin login                                                     | admin@govsense.com |
| PGADMIN_DEFAULT_PASSWORD   | Password for pgAdmin login                                                  | govsense           |
...
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| DATABASE_URL               | PostgreSQL connection string (e.g., `postgresql+asyncpg://postgres:postgres@localhost:5432/govsense`)                                                                                    |
...
| LANGSMITH_PROJECT | LangSmith project name (e.g., `govsense`) |
...
2. Login with the credentials from your `.env` file (default: admin@govsense.com / govsense)
3. Right-click "Servers" > "Create" > "Server"
4. In the "General" tab, name your connection (e.g., "GovSense DB")
5. In the "Connection" tab:
   - Host: `db`
   - Port: `5432`
   - Maintenance database: `govsense`
...
Once your installation is complete, you can start using GovSense! Navigate to the frontend URL and log in using your Google account.
