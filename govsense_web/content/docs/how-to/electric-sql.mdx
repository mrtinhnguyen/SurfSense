...
The simplest way to run GovSense with Electric SQL is using the all-in-one Docker image. This bundles everything into a single container:
...
Access GovSense at `http://localhost:3000`. Electric SQL is automatically configured and running on port 5133.
...
electric:
  ...
  environment:
    - DATABASE_URL=${ELECTRIC_DATABASE_URL:-postgresql://${ELECTRIC_DB_USER:-electric}:${ELECTRIC_DB_PASSWORD:-electric_password}@${POSTGRES_HOST:-db}:${POSTGRES_PORT:-5432}/${POSTGRES_DB:-govsense}?sslmode=disable}
...
Ensure your environment files are configured. If you haven't set up GovSense yet, follow the [Manual Installation Guide](/docs/manual-installation) first.
...
2. Delete databases starting with `govsense-`
