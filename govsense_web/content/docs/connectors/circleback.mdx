---
title: Circleback
description: Connect your Circleback meetings to GovSense
---

# Circleback Integration Setup Guide

This guide walks you through connecting your Circleback meetings to GovSense for meeting search and AI-powered insights.

## How it works

The Circleback connector uses a **webhook-based integration**. Unlike other connectors that pull data from APIs, Circleback automatically pushes meeting data to GovSense when meetings are processed.

- Meetings are sent to GovSense immediately after Circleback processes them
...
<Callout type="info">
    Meeting recordings are not stored in GovSense, but a link to the recording (valid for 24 hours from Circleback) is included in the indexed document.
</Callout>
...
### Step 1: Create the Connector in GovSense
...
```
https://your-govsense-url/api/v1/webhooks/circleback/{search_space_id}
```
...
6. Enter your GovSense webhook URL
...
<Callout type="warn">
    Make sure your GovSense backend is accessible from the internet for Circleback to send webhooks. If self-hosting, you may need to configure a reverse proxy or use a tunneling service.
</Callout>
...
<Callout type="info">
    Unlike other connectors, Circleback doesn't support periodic indexing since it's webhook-based. Meetings are automatically pushed to GovSense when processed by Circleback.
</Callout>
...
Once configured, new meetings will automatically appear in GovSense after Circleback processes them. To verify:
...
3. Check your GovSense search space for the new meeting document
...
- Check that your GovSense backend is accessible from the internet
- Look for errors in your GovSense backend logs
