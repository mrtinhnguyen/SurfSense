"""
Streaming task for the new GovSense deep agent chat.
...
from app.agents.new_chat.chat_deepagent import create_govsense_deep_agent
...
from app.db import Document, GovSenseDocsDocument
...
def format_mentioned_govsense_docs_as_context(
    documents: list[GovSenseDocsDocument],
) -> str:
...
async def stream_new_chat(
    ...
    """
    Stream chat responses from the new GovSense deep agent.
    ...
    try:
        ...
        agent = await create_govsense_deep_agent(
            llm=llm,
            search_space_id=search_space_id,
            db_session=session,
        ...
        mentioned_govsense_docs: list[GovSenseDocsDocument] = []
        if mentioned_govsense_doc_ids:
            ...
            result = await session.execute(
                ...
                .filter(
                    GovSenseDocsDocument.id.in_(mentioned_govsense_doc_ids),
                )
            )
