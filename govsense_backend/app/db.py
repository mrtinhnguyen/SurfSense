...
INCENTIVE_TASKS_CONFIG = {
    IncentiveTaskType.GITHUB_STAR: {
        ...
        "description": "Show your support by starring GovSense on GitHub",
        "pages_reward": 100,
        "action_url": "https://github.com/MODSetter/GovSense",
    },
    IncentiveTaskType.REDDIT_FOLLOW: {
        "title": "Join our Subreddit",
        "description": "Join the GovSense community on Reddit",
        "pages_reward": 100,
        "action_url": "https://www.reddit.com/r/GovSense/",
    },
    IncentiveTaskType.DISCORD_JOIN: {
        "title": "Join our Discord",
        "description": "Join the GovSense community on Discord",
        "pages_reward": 100,
    ...
    #     "description": "Help improve GovSense by reporting bugs or suggesting features",
    #     "pages_reward": 50,
    #     "action_url": "https://github.com/MODSetter/GovSense/issues/new/choose",
    # },
}
...
class GovSenseDocsDocument(BaseModel, TimestampMixin):
    """
    GovSense documentation storage.
    Indexed at migration time from MDX files.
    """

    __tablename__ = "govsense_docs_documents"
...
class GovSenseDocsChunk(BaseModel, TimestampMixin):
    """Chunk storage for GovSense documentation."""

    __tablename__ = "govsense_docs_chunks"
    ...
    document_id = Column(
        Integer,
        ForeignKey("govsense_docs_documents.id", ondelete="CASCADE"),
        nullable=False,
    )
...
class NewLLMConfig(BaseModel, TimestampMixin):
    ...
    - Configurable system instructions (defaults to GOVSENSE_SYSTEM_INSTRUCTIONS)
    - Citation toggle (enable/disable citation instructions)

    Note: GOVSENSE_TOOLS_INSTRUCTIONS is always used and not configurable.
    """
    ...
    system_instructions = Column(
        Text,
        nullable=False,
        default="",  # Empty string means use default GOVSENSE_SYSTEM_INSTRUCTIONS
    )
    ...
    # Citation toggle - when enabled, GOVSENSE_CITATION_INSTRUCTIONS is injected
...
async def setup_indexes():
    async with engine.begin() as conn:
        ...
        await conn.execute(
            text(
                "CREATE INDEX IF NOT EXISTS idx_govsense_docs_title_trgm ON govsense_docs_documents USING gin (title gin_trgm_ops)"
            )
        )
