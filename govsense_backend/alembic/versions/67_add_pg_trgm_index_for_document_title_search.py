...
4. idx_govsense_docs_title_trgm - GIN trigram on govsense docs title
...
def upgrade() -> None:
    ...
    # 4. GIN trigram index on govsense_docs_documents.title
    op.execute(
        """
        CREATE INDEX IF NOT EXISTS idx_govsense_docs_title_trgm 
        ON govsense_docs_documents USING gin (title gin_trgm_ops);
        """
    )
...
def downgrade() -> None:
    ...
    op.execute("DROP INDEX IF EXISTS idx_govsense_docs_title_trgm;")
